...
fun main() {
    Solution(2021, 12)
        .part1 {
            ...
            findPaths(connections, visited = setOf("start"), "start", true).size
        }
        .part2 {
            val connections = mutableMapOf<String, MutableList<String>>()
            ...
            findPaths(connections, visited = setOf("start"), "start", false).size
        }
}
    ...
    connections[loc]?.forEach { neighbor ->
        if (neighbor.lowercase() == neighbor && visited.contains(neighbor)) {
            if (!twice && neighbor != "start") {
                out.addAll(findPaths(connections, visited + setOf(loc), neighbor, true)
                    .map { listOf(loc) + it })
            }
        ...
        out.addAll(findPaths(connections, visited + setOf(loc), neighbor, twice)
            .map { listOf(loc) + it })
    }
